# Trigger: Forçando nova avaliação do workflow
...
resource "aws_instance" "test_instance" {
  # Usando o valor dinâmico obtido do SSM Parameter Store
...
# 2. POLÍTICA DO S3 SEPARADA: Resolve a referência circular.
...
# RECURSO NULL_RESOURCE: Força uma pausa de 40 segundos após a criação da chave KMS.
...
resource "aws_cloudtrail" "audit_trail" {
  ...
  # Dependência FINAL: Aguarda as políticas e o período de espera.
  depends_on = [
